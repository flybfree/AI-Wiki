# Agentic Fusion of Large Atomic and Language Models to Accelerate Materials Discovery
Saved: 2026-05-08 03:29
Source: 2026-04-26_15-14-38Z_AgenticFusionofLargeAtomicandLanguageModelstoAccel.md

---

## Summary
Introduces ElementsClaw, an agentic framework that combines large atomic models with large language models to orchestrate materials-discovery workflows. In superconductors, it guided the synthesis of four new superconductors and screened more than 2.4 million stable crystals to identify 68,000 high-confidence superconducting candidates.

## Key Takeaways
- Pairs LAM tool use with LLM reasoning for human-interactive discovery.
- Demonstrates both experimental synthesis guidance and large-scale screening.
- Targets integrated materials discovery rather than isolated prediction or generation.

## Context
The abstract frames materials discovery as a workflow problem that needs both atomic-scale computation and semantic orchestration.

## Implications
The approach suggests a way to accelerate superconductors research while maintaining high physical fidelity.

## Original Reference
- Title: Agentic Fusion of Large Atomic and Language Models to Accelerate Materials Discovery
- Authors: Mingze Li, Yu Rong, Songyou Li, Lihong Wang, Jiacheng Cen, Liming Wu, Anyi Li, Zongzhao Li, Qiuliang Liu, Rui Jiao, Tian Bian, Pengju Wang, Hao Sun, Jianfeng Zhang, Ji-Rong Wen, Deli Zhao, Shifeng Jin, Tingyang Xu, Wenbing Huang
- Published: 2026-04-26T15:14:38Z
- URL: http://arxiv.org/abs/2604.23758v1
- Source file: /home/rich/wiki/ai-research/raw/papers/2026-04-26_15-14-38Z_AgenticFusionofLargeAtomicandLanguageModelstoAccel.md
