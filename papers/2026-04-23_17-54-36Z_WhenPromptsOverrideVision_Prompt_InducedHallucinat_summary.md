# Summary: 2026-04-23_17-54-36Z_WhenPromptsOverrideVision_Prompt_InducedHallucinat
Saved: 2026-05-08 03:29
Source: 2026-04-23_17-54-36Z_WhenPromptsOverrideVision_Prompt_InducedHallucinat.md
Model: None

---

## Summary
HalluScope is a benchmark for studying hallucinations in large vision-language models, focusing on how textual priors and instructions override visual grounding. The paper also introduces HalluVL-DPO, a preference-optimization method that encourages grounded responses while preserving broader model performance.

## Key Takeaways
- Finds that many hallucinations are driven by textual instructions and background knowledge.
- Provides a curated preference dataset for grounding-focused fine-tuning.
- Improves the targeted failure mode without sacrificing other measured capabilities.


## Context
The paper investigates why LVLMs produce visually ungrounded answers despite strong multimodal performance.

## Implications
Instruction-driven hallucinations can be reduced with targeted preference optimization.

## Original Reference
- Title: When Prompts Override Vision: Prompt-Induced Hallucinations in LVLMs
- Authors: Pegah Khayatan, Jayneel Parekh, Arnaud Dapogny, Mustafa Shukor, Alasdair Newson, Matthieu Cord
- URL: http://arxiv.org/abs/2604.21911v1
- Published: 2026-04-23T17:54:36Z
- Source file: /home/rich/wiki/ai-research/raw/papers/2026-04-23_17-54-36Z_WhenPromptsOverrideVision_Prompt_InducedHallucinat.md

[[When Prompts Override Vision: Prompt-Induced Hallucinations in LVLMs]]