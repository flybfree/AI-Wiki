...
- **ROMS-IMLE** argues that strong image generation does not have to depend on iterative denoising.
- **Appearance Pointers** adds multimodal region control to diffusion transformers without retraining the base model.
...
- **CopyLess, GroundMore** attacks repetitive copying in long-context reasoning.
- **CodeRescue** frames coding-agent recovery as a budgeted routing problem.
- **RLAES** ties essay scoring and feedback generation to rubric-based RL.
...
- **MeetingToM** evaluates theory-of-mind in multimodal meetings, which is a useful proxy for social reasoning in agent-like systems.
...
- **In-context time series classification** shows how foundation-model style inference can work without task-specific training.
...
- **Thermodynamics-Informed Input Reparameterization** improves neural surrogates for real-fluid simulation.
- **1-Lipschitz Neural Networks on Hadamard Manifolds** focuses on geometry-aware robustness.
- **Associative Emotional Learning** shows a CNN-style model of valence conditioning.
