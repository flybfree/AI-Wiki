...
The curation pass promoted four pending summaries, but one—*Do Frontier Models Seek Safety Evidence Before Acting?*—contains no usable abstract or results and is not included as evidence. The three usable papers sharpen today’s systems theme. [Coaching Qwen3 Coder 30B](https://arxiv.org/abs/2609.16096v1) reports that ReAct SFT (Supervised Fine-Tuning using observation–thought–action traces) and trajectory-quality weighting improved an open-weight coding agent’s strategic behavior. [Benchmarking Factual Robustness of LLMs via Multi-conversation Persuasion](https://arxiv.org/abs/2609.16777v1) introduces SAST-IR, which wipes the target’s memory while retaining attacker history, and reports an 84.7% genuine-persuasion rate for simple attacks in its test setting. [Agentic Societies Need a Social Harness](https://arxiv.org/abs/2609.17527v1) argues that inter-agent communication needs a separate control layer for invalid-message detection, prevention, and post-facto accountability.

**Why it matters:** these papers point to three distinct control surfaces—training trajectories, memory boundaries, and social protocols. They are promising mechanisms, not production guarantees: the reported results need replication, clearer baselines, and testing outside the authors’ environments. Their inclusion also demonstrates the curation boundary: the malformed fourth paper is retained in local records but excluded from the briefing.
...
- [Coaching Qwen3 Coder 30B — arXiv](https://arxiv.org/abs/2609.16096v1)
- [Benchmarking Factual Robustness via Multi-conversation Persuasion — arXiv](https://arxiv.org/abs/2609.16777v1)
- [Agentic Societies Need a Social Harness — arXiv](https://arxiv.org/abs/2609.17527v1)
