...
async function load(){try{
 const [qResponse,pResponse]=await Promise.all([
  fetch('/api/candidates?status='+status+'&limit=50&offset='+(page*50),{cache:'no-store'}),
  fetch('/api/profile',{cache:'no-store'})
 ]);
 if(!qResponse.ok||!pResponse.ok)throw new Error('API request failed');
 const [q,p]=await Promise.all([qResponse.json(),pResponse.json()]);
 ...
 }catch(error){console.error(error);document.querySelector('#queue').innerHTML='<div class="empty">Unable to load the curation queue. Click Refresh to retry.</div>';}}
