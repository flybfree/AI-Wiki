...
from curation import delete_rejected, list_candidates, profile, record_decision
...
async function load(){const [q,p]=await Promise.all([fetch('/api/candidates?status='+status+'&limit=50&offset='+(page*50)).then(r=>r.json()),fetch('/api/profile').then(r=>r.json())]);
 ...
 document.querySelector('#queue').innerHTML=q.length?q.map(card).join(''):'<div class="empty">No summaries in this view.</div>';
