...
source_url: https://alyssarosenzweig.ca/blog/hilariously-fast-volume-computation-with-the-divergence-theorem.html
source_feed: Hacker News
ai_relevance: include
ai_topic: benchmark-eval
ai_reason: meets AI relevance threshold
scraped: 2026-08-28 05:30
---
...
d
A
V = \sum_{i = 0} \iint_{T_i} \mathbf{F}(x, y, z) (\mathbf{r}_u \times \mathbf{r}_v) dA
=
∑
i
=
...
d
A
= \sum_{i = 0} \iint_{T_i} \mathbf{F}(x, y, z) \dot (\Delta_{i1} \times \Delta_{i2}) dA
=
∑
i
=
...
d
A
= \sum_{i = 0} \iint_{T_i} <x, 0, 0> \dot (\Delta_{i1} \times \Delta_{i2}) dA
...
d
u
\int_{0}^{1} \int_{0}^{1-u} (T_{i0x} + u \Delta_{i1x} + v \Delta_{i2x}) dv du
=
T
i
0
...
d
u
= T_{i0x} \int_{0}^{1} \int_{0}^{1-u} dv du + \Delta_{i1x} \int_{0}^{1} \int_{0}^{1-u} u dv du + \Delta_{i2x}) \int_{0}^{1} \int_{0}^{1-u} v dv du
=
T
i
0
...
= T_{i0x} (\frac{1}{6}) + (T_{i1x})(\frac{1}{6}) + (T_{i2x})(\frac{1}{6})
=
1
6
(
...
n
11n
floating point operations. This is
very
fast.
...
of a GPU, only using the CPU capabilities of a
$35
Raspberry Pi
, around 30 million triangles could be measured every
frame.
Motivation
...
Further research
after
posting reveals the paper
Efficient
Feature Extraction for 2D/3D Objects in Mesh Representation
by Cha
...
## Metadata
- **Source**: [Original Article](https://alyssarosenzweig.ca/blog/hilariously-fast-volume-computation-with-the-divergence-theorem.html)
