...
Who would have thought a single colon would be the one to save the
day
night?
Note
: Want your mind blown straight away? See
more colons in the limelight
.
Checking for required arguments
#
...
identifying that one part is using
parameter expansion
:
The syntax
${name:?diagnostic}
...
$name
is unset or empty
— if it is, the diagnostic is printed to
stderr
and the shell exits with a
non-zero status, otherwise;
if the variable is set, it is equivalent to
$name
.
That.. other colon
#
...
result.
:
is old — it goes all the way back to the
1971 Thompson
shell
where it doubled as a
label
and Unix's very first
comment marker.
:
...
:
than meets the
eye, but to prove the real magic of the
null-command
—
...
Note
: The above example is safe to run locally, try it!
Frequently Asked Questions
#
...
Why do I need the null-command? Doesn't the expansion happen without the colon?
#
The parameter expansion will happen regardless, but without a null-command or
similar usage the shell will treat the resulting string as a command to run.
%
${HELLO:=123}
zsh: command not found: 123
If we prefix our parameter-expansion with the null-command, the result is
discarded, but the expression is still evaluated (setting
HELLO
to
123
).
...
?
#
This at its core boils down to personal preference, but using our beloved
colon we can shrink the number of potential typos to one (rather than two):
:
"
${DATA_DIR:=/var/data}
"
# <- DATA_DIR mentioned once (1)
DATA_DIR=
"
${DATA_DRI:-/var/data}
"
...
## Metadata
- **Source**: [Original Article](https://refp.se/articles/your-shell-and-the-magic-colon)
