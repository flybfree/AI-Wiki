...
For
image_url
inputs you can optionally set a
detail
field to control how the image is processed:
Value
Behavior
low
The image is downscaled to 512Ã512 before inference. Faster and cheaper when fine visual detail is not important.
high
...
â
Inline images (base64 or
file_data
) count toward the request body size limit of
48 MiB
. Consider the
Files API
when:
...
Images with a total pixel count below roughly 384Ã384 are scaled up while preserving their aspect ratio.
...
384
tokens per image: for example, a 2000Ã2000 image and a 5000Ã5000 image consume the same number of tokens after resizing. When a request contains multiple images, each image is counted independently under the same rule â there is no separate calculation for multi-image requests.
...
For storage and upload quotas of files uploaded via the Files API, see
Files API: Limits
.
Restrictions
â
...
Only vision models (
deepseek-v4-flash-vision-exp
) accept images; other models return a
400
error ("This model does not support image").
User text containing the reserved image placeholder token is rejected with a
400
error.
...
limits
apply; only the content part shape differs â images are carried in
input_image
parts, either in
user
/
...
## Metadata
- **Source**: [Original Article](https://api-docs.deepseek.com/guides/vision/)
