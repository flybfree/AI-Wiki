...
source_url: https://github.com/wie-project/kakehashi
source_feed: Hacker News
ai_relevance: include
ai_topic: safety-governance
ai_reason: meets AI relevance threshold
scraped: 2026-08-02 14:00
---
...
translation layer (CLI-first, no JIT).
Load Darwin Mach-O on Linux aarch64, map a freestanding
libSystem
, translate
BSD syscalls, and run real guests (clang probes,
7-Zip
7zz
,
curl
...
Relative
-o
/ archive paths resolve against the
host CWD
of the
kh
process (create parent dirs yourself, or rely on auto-mkdir for
O_CREAT
).
Through the bottle,
/Volumes/linux/…
...
— OpenSSL optional config; HTTP/HTTPS still work via the seeded CA bundle.
WARN … skip dylib … Security/CoreFoundation
— Apple frameworks not in the bottle; soft stubs cover the load path.
...
The guest dylib is vendored at
crates/kh-runtime/resources/libSystem.B.dylib
and compiled into the runtime with
include_bytes!
. Publishing
kh-runtime
...
→ host FS
Performance (honest)
Kakehashi runs guest code
natively
on the CPU. The tax is the
syscall
boundary
(TLS switch, alt stack, NEON save/restore, Rust dispatch) × how
chatty the guest is — not an instruction emulator.
Measured gap
On Ubuntu aarch64 bare-metal (UTM), multi-file
7zz
archive
(
...
(~×1.1–1.2). The large multi-file gap is dominated by
path walk + per-syscall
boundary
, not “wrong LZMA”.
Hypercall is on by default for all guest threads. Opt out with
KAKEHASHI_HYPERCALL=0
only for debug (residual
svc
→
brk
/ SIGTRAP).
Why ~×5 is still useful in CI
The product goal for CI is not “as fast as native macOS,” but
run Darwin
CLI/tools on cheap Linux aarch64 runners
instead of scarce, expensive
macOS capacity.
GitHub Actions hosted runners
(private-repo overage rates, USD per
minute; see
...
$0.077–$0.102
macOS standard is roughly
×10–×12
the Linux arm64 minute rate before any
wall-time difference. Even if a job runs
×5 slower
under
kh
on Linux
...
When macOS runners still win:
GUI, codesign/notarization, Xcode UI tests,
...
Bottle bridges the Linux FS as
/Volumes/linux/…
:
Guest path
Host
...
This project is
not
derived from Darling. Do not vendor proprietary Apple
SDKs or blobs.

## Metadata
- **Source**: [Original Article](https://github.com/wie-project/kakehashi)
