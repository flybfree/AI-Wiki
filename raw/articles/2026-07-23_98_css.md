...
, it merely styles your HTML with some CSS.
...
Note: You
must
include a corresponding label
after
your checkbox, using the
<label>
element with a
for
attribute
...
, also referred to as a radio button, represents a single
          ...
          one set of options.
Option buttons can be used via the
radio
...
Text boxes can rendered by specifying a
text
type on an
input
element. As with checkboxes and radio buttons, you
          should provide a corresponding label with a properly set
for
attribute, and wrap both in a container with the
field-row
class.
Occupation
Show code
...
Sliders can rendered by specifying a
range
type on an
input
element.
...
allows the selection of only a
          ...
          the value.
Dropdowns can be rendered by using the
select
and
option
elements.
...
A status bar is a special area within a window, typically the bottom, that displays information
          ...
          state.
You can render a status bar with the
status-bar
class,
          and
status-bar-field
...
To render a tree view, use an
ul
element with the
tree-view
class. The children of this list (
li
...
ul
>
To make this a tree, we can nest further
ul
elements
...
element with the
[role=tablist]
attribute. The children of this menu (
li
elements), should get a
[role=tab]
attribute.
...
div
>
To create multirows tabs, add a
multirows
class to the
menu
tag.
...
>
TableView
To render a table view, use a table element. Wrap it with a div element with
sunken-panel
class to provide proper border and overflow container.
With a bit of extra scripting you can make table view interactive. Give
interactive
class to
...
## Metadata
- **Source**: [Original Article](https://jdan.github.io/98.css/#status-bar)
